...
def get_data(device = 'cpu'):
    ...
    all_images = torch.cat([ dataset["train_images"], dataset["test_images"] ], dim = 0)
    all_labels = torch.cat([ dataset["train_labels"].to(device), dataset["test_labels"].to(device) ], dim = 0)
    return all_images, all_labels

def random_images_and_labels(all_images, all_labels, num_pts = 100):
    N = all_labels.shape[0]
    idx = torch.randperm(N)[:num_pts]
    image_sample = all_images[idx]
    label_sample = all_labels[idx]
    return image_sample, label_sample

def evaluate_model(model = None, all_images= None, all_labels = None, num_evals = 16, num_pts = 512):
    if model is None:
        model = get_model()
    if all_images is None or all_labels is None:
        all_images, all_labels = get_data('cpu')
    output = []
    for _ in range(num_evals):
        image_sample, label_sample = random_images_and_labels(all_images, all_labels, num_pts = num_pts)
        evals = model(image_sample).squeeze()
        evals = torch.softmax(evals, dim = -1)
        evals = evals[torch.arange(evals.shape[0]), label_sample]
        output.append(evals.squeeze())
    ...
    with torch.no_grad():
        ...
        for fid in fid_list:
            ...
            for _ in range(num_reps):
                evals = evaluate_model(model, all_images= all_images, all_labels = all_labels, num_evals = fid, num_pts = 64)
