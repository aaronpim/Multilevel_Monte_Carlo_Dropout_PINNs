...
def get_inputs_and_labels(N = 200):
    CONFIG = make_config()
    _, _, hold_loader  = load_all(CONFIG, device = 'cpu')
    images = []
    labels = []
...
def reference_mean_and_variance(seed = 0, batch_size = 1000, num_batches = 50, filename = None):
    '''
    ...
    '''
    if filename is None:
        filename = f'plots/reference_estimator_T_{batch_size*num_batches}.pt'
    with torch.no_grad ():
        set_seed(seed=seed)
        ...
        for i in range(num_batches):
            evals = torch.stack([model(images).squeeze() for _ in range(batch_size)])
            evals   = torch.softmax(evals, dim = -1)
            evals = evals[:,torch.arange(evals.shape[1]), labels]
...
def load_reference_mean_and_variance(seed = 0, batch_size = 1000, num_batches = 50, filename = None):
    if filename is None:
        filename = f'plots/reference_estimator_T_{batch_size*num_batches}.pt'
...
def fixed_cost_RMSE(master_seed = 0, num_reps = 50, fidelity_ladder =[4, 8, 16], base_cost = 50, cost_multiplier = [1,2,4,8,16], log_paths = ['plots/time_v_cost.csv', 'plots/RMSE_v_cost.csv']):
    with torch.no_grad():
        ...
        ref_sample_var = ref_sample_var.mean()
        for C in cost_multiplier:
            ...
            for seed in range(num_reps):
                set_seed(seed = seed)
                start_time = time.time()
                mean_estimator, _, sample_variance_estimator, _ = multi_level_multi_fidelity_eval(model, images, labels, fidelity_ladder = fidelity_ladder, num_levels = num_levels)
...
# Runtime exhibited approximately linear scaling with cost over the tested range. A linear regression explained 99.7% of the variance across all runs (R2=0.9999480547436692), while regression on the median runtime at each cost yielded R2=0.9996482867556904.
