...
from src.MLMC_base import make_config, MLMC_config, multi_level_multi_fidelity_eval, levels_to_num_evals
...
def reference_mean_and_variance(seed = 0, batch_size = 1000, num_batches = 100, filename = None):
    '''
...
def load_reference_mean_and_variance(seed = 0, batch_size = 1000, num_batches = 100, filename = None):
    if filename is None:
        filename = f'plots/reference_estimator_T_{batch_size*num_batches}.pt'
...
def fixed_cost_RMSE(master_seed = 0, num_reps = 50,  x_num_reduced = 51, eps_num_reduced = 101, fidelity_ladder =[4, 8, 16], base_cost = 50, cost_multiplier = [1,2,4,8,16], log_paths = ['plots/time_v_cost.csv', 'plots/RMSE_v_cost.csv']):
    with torch.no_grad():
        clock_vec = []
        RMSE_vec  = []
        ...
        ref_mean = factor*ref_mean.sum()
        ref_sample_var = factor*ref_sample_var.sum()
...
# Runtime exhibited approximately linear scaling with cost over the tested range. A linear regression explained 99.7% of the variance across all runs (R2=0.9995621100719373), while regression on the median runtime at each cost yielded R2=0.9797901530662747.
