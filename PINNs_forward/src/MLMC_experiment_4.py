...
from src.MLMC_base import MLMC_config, make_config
from src.trainmodel import set_seed, CONFIG_to_folder_path
from src.model_defn import load_PirateNet

def gen_x_and_eps(CONFIG = None, device = 'cpu'):
    if CONFIG is None:
        CONFIG = MLMC_config()
    x = torch.rand(CONFIG["x_num"]).to(device)
    eps = torch.rand(CONFIG["eps_num"]).to(device)*(CONFIG["eps_max"]-CONFIG["eps_min"]) + CONFIG["eps_min"]
    X, E = torch.meshgrid(x,eps, indexing = 'ij')
    input_pairs = torch.stack([X, E], dim = -1)
    return input_pairs
...
def evaluate_model(model = None, num_evals = 16):
    if model is None:
        model = get_model()
    output = []
    for _ in range(num_evals):
        input_pairs = gen_x_and_eps()
