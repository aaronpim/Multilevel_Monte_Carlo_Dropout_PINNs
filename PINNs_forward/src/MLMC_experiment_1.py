...
def get_estimator_variance_vary_cost(num_models = 51, fidelity_ladder =[4, 8, 16], base_cost = 50, cost_multiplier = [1,2,4,8,16], log_path = 'plots/estimator_variance_vary_cost.csv', x_num_reduced = 51, eps_num_reduced = 101):
    output_vec  = []
    for C in cost_multiplier:
        ...
        for seed in range(num_models):
            ...
            _, mean_estimator_variance, _, sample_variance_estimator_variance = multi_level_multi_fidelity_eval(model, values, fidelity_ladder = fidelity_ladder, num_levels = num_levels)
