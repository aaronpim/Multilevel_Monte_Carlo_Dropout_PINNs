...
def prepare_experiment_directory(run_dir, fidelities, levels, experiment_number = 1, u_or_f = 'f'):
    if u_or_f == 'f':
        experiment_dir = os.path.join(run_dir, f"experiment_{experiment_number}_f")
    else:
        experiment_dir = os.path.join(run_dir, f"experiment_{experiment_number}_u")
...
def initialise(fidelities = [2,4,8,16,32], levels = [9,7,5,3,1], u_or_f = 'f', if_config = False):
    model, x, CONFIG, run_dir = load_latest_model(device = "cpu")
    exact = exact_solution(CONFIG, device = "cpu",  u_or_f = u_or_f)
    exp_estimator_exp, exp_estimator_var, var_estimator_exp, var_estimator_var = get_estimator(model, x, fidelities, levels, u_or_f = u_or_f)
    x = x.numpy().flatten()
    ...
    if not if_config:
        ...
        return exp_estimator_exp, exp_estimator_var, var_estimator_exp, var_estimator_var, x, exact, run_dir, CONFIG

def experiment_1_output_mean_and_variance(fidelities = [2,4,8,16,32], levels = [9,7,5,3,1], u_or_f = 'f'):
    exp_estimator_exp, _, var_estimator_exp, _, x, exact, run_dir, CONFIG = initialise(fidelities, levels, u_or_f = u_or_f, if_config = True)
    experiment_subdir = prepare_experiment_directory(run_dir, fidelities, levels, experiment_number = 1, u_or_f = u_or_f)
    hash_input = json.dumps({"fidelities": fidelities, "levels": levels}, sort_keys=True)
    unique_hash = hashlib.sha256(hash_input.encode()).hexdigest()[:12] + u_or_f

    var_exact = ((CONFIG["random_eps_width"])**2/12)*exact**2
    if u_or_f == 'f':
        plt.figure(figsize=(8,5))
        plt.plot(x, exact, label=r'$\mathbb{E}[f(x)]$', linestyle='dashed', color = 'black')
        plt.plot(x , exp_estimator_exp, label=r'$\mathcal{Y}(x;\mathscr{F})$', color = 'orange')
        plt.grid(True)
        plt.xlabel('x')
        plt.legend()
        plt.tight_layout()
        save_path = os.path.join(experiment_subdir, f"expecation_v_exact_{unique_hash}.pdf")
        plt.savefig(save_path)
        plt.close()

        plt.figure(figsize=(8,5))
        plt.plot(x, np.abs(exact-exp_estimator_exp), color = 'black')
        plt.grid(True)
        plt.xlabel('x')
        plt.ylabel(r'$|\mathcal{Y}(x;\mathscr{F})-\mathbb{E}[f(x)]|$')
        plt.tight_layout()
        save_path = os.path.join(experiment_subdir, f"expecation_error_{unique_hash}.pdf")
        plt.savefig(save_path)
        plt.close()

        plt.figure(figsize=(8,5))
        plt.plot(x, np.sqrt(var_exact), label=r'$\sqrt{\mathrm{Var}}[f(x)]$', linestyle='dashed', color = 'black')
        plt.plot(x, np.sqrt(var_estimator_exp), label=r'$\sqrt{\mathcal{V}}(x;\mathscr{F})$.', color = 'orange')
        plt.grid(True)
        plt.xlabel('x')
        plt.legend()
        plt.tight_layout()
        save_path = os.path.join(experiment_subdir, f"var_v_exact_{unique_hash}.pdf")
        plt.savefig(save_path)
        plt.close()

        plt.figure(figsize=(8,5))
        plt.plot(x, np.abs(np.sqrt(var_exact)-np.sqrt(var_estimator_exp)), color = 'black')
        plt.grid(True)
        plt.xlabel('x')
        plt.ylabel(r'$|\sqrt{\mathcal{V}}(x;\mathscr{F})-\sqrt{\mathrm{Var}}[f(x)]|$')
        plt.tight_layout()
        save_path = os.path.join(experiment_subdir, f"variance_error_{unique_hash}.pdf")
        plt.savefig(save_path)
        plt.close()
    else:
        plt.figure(figsize=(8,5))
        plt.plot(x, exact, label=r'$\mathbb{E}[u(x)]$', linestyle='dashed', color = 'black')
        plt.plot(x , exp_estimator_exp, label=r'$\mathcal{Y}(x;\mathscr{U})$', color = 'orange')
        plt.grid(True)
        plt.xlabel('x')
        plt.legend()
        plt.tight_layout()
        ...
        plt.grid(True)
        plt.xlabel('x')
        plt.ylabel(r'$|\mathcal{Y}(x;\mathscr{U})-\mathbb{E}[u(x)]|$')
        plt.tight_layout()
        save_path = os.path.join(experiment_subdir, f"expecation_error_{unique_hash}.pdf")
        plt.savefig(save_path)
        plt.close()
        ...
        plt.figure(figsize=(8,5))
        plt.plot(x, np.sqrt(var_exact), label=r'$\sqrt{\mathrm{Var}}[u(x)]$', linestyle='dashed', color = 'black')
        plt.plot(x, np.sqrt(var_estimator_exp), label=r'$\sqrt{\mathcal{V}}(x;\mathscr{U})$.', color = 'orange')
        plt.grid(True)
        plt.xlabel('x')
        plt.legend()
        plt.tight_layout()
        ...
        plt.grid(True)
        plt.xlabel('x')
        plt.ylabel(r'$|\sqrt{\mathcal{V}}(x;\mathscr{U})-\sqrt{\mathrm{Var}}[u(x)]|$')
        plt.tight_layout()
        save_path = os.path.join(experiment_subdir, f"variance_error_{unique_hash}.pdf")
        plt.savefig(save_path)
        plt.close()
...
def experiment_2_exp_estimator_var_vs_fid(
    ...
    results = []
    model, x, CONFIG, run_dir = load_latest_model(device = "cpu")
    experiment_subdir = prepare_experiment_directory(run_dir, fidelities, [level], experiment_number = 2, u_or_f = u_or_f)
    ...
    if os.path.exists(npz_file):
        ...
        for f in fidelities:
            _, exp_estimator_var, _, _ = get_estimator(model, x, f, [level], u_or_f = u_or_f)
    ...
    if u_or_f == 'f':
        plt.ylabel(r'$\int S_Y^2(x;\mathscr{F})~ dx$')
    else:
        plt.ylabel(r'$\int S_Y^2(x;\mathscr{U})~ dx$')
    plt.xlabel('T')
    plt.grid(True)
    plt.tight_layout()
    save_path = os.path.join(experiment_subdir, f"exp_estimator_var_v_fid_{u_or_f}.pdf")
...
def experiment_3_var_estimator_var_vs_fid(
    ...
    results = []
    model, x, CONFIG, run_dir = load_latest_model(device = "cpu")
    experiment_subdir = prepare_experiment_directory(run_dir, fidelities, [level], experiment_number = 3, u_or_f = u_or_f)
    npz_file = os.path.join(experiment_subdir, "results.npz")
    if os.path.exists(npz_file):
        print(f"Results already exist. Loading from: {npz_file}")
        data = np.load(npz_file)
        ...
        for f in fidelities:
            _, _, _, var_estimator_var = get_estimator(model, x, f, [level], u_or_f = u_or_f)
    ...
    plt.fill_between(fidelities_flat, fit_lower, fit_upper, color='gray', alpha=0.3, label='99% CI')
    if u_or_f == 'f':
        plt.ylabel(r'$\int S_V^2(x;\mathscr{F})~ dx$')
    else:
        plt.ylabel(r'$\int S_V^2(x;\mathscr{U})~ dx$')
    plt.xlabel('T')
    plt.grid(True)
    plt.tight_layout()
    save_path = os.path.join(experiment_subdir, f"var_estimator_var_v_fid_{u_or_f}.pdf")
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
    return None
...
def experiment_4_variance_v_dropout_prob(
    ...
    model, x, CONFIG, run_dir = load_latest_model(device = "cpu")
    experiment_subdir = prepare_experiment_directory(run_dir, sum([fidelities, levels],[]), list(p_vec), experiment_number = 4, u_or_f = u_or_f)
    npz_file = os.path.join(experiment_subdir, "results.npz")
    ...
    if os.path.exists(npz_file):
        ...
        for p in p_vec:
            ...
            _, exp_estimator_var, _, var_estimator_var = get_estimator(model, x, fidelities, levels, u_or_f = u_or_f)
    ...
    plt.grid(True)
    plt.tight_layout()
    save_path = os.path.join(experiment_subdir, f"exp_estimator_var_v_pdrop_{u_or_f}.pdf")
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    save_path = os.path.join(experiment_subdir, f"var_estimator_var_v_pdrop_{u_or_f}.pdf")
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
    return None
...
def experiment_5_var_v_levels_fixed_cost(c = 1000, T = [4,8,16], u_or_f = 'f'):
    model, x, CONFIG, run_dir = load_latest_model(device = "cpu")
    experiment_subdir = prepare_experiment_directory(run_dir, [c], T, experiment_number = 5, u_or_f = u_or_f)
    npz_file = os.path.join(experiment_subdir, "results.npz")
    ...
    if os.path.exists(npz_file):
        ...
        for i, M in enumerate(M_all):
            ...
            _, exp_estimator_var, _, var_estimator_var = get_estimator(model, x, T, M, u_or_f = u_or_f)
...
def experiment_6_dropout_bias(
    ...
    u_or_f = 'f'):
    model, x, CONFIG, run_dir = load_latest_model(device = "cpu")
    experiment_subdir = prepare_experiment_directory(run_dir, fidelities+levels , [num_drop], experiment_number = 6, u_or_f = u_or_f)
    npz_file = os.path.join(experiment_subdir, f"results.npz")
    exact = exact_solution(CONFIG, device = "cpu",  u_or_f = u_or_f)
    exact = exact.numpy()
    ...
    if os.path.exists(npz_file):
        ...
        if u_or_f == 'f':
            ...
            _, f0 = model(x)
            out = f0.detach().numpy()
        exp_v_p = [np.sqrt(np.mean((out - exact)**2))]
        var_v_p = [0.0]
        model.train()
        p_vec  = np.linspace(0,1,num_drop+1, endpoint= False)[1:]
        for p in p_vec:
            set_dropout_probability(model, p)
            exp_estimator_exp, _, var_estimator_exp, _ = get_estimator(model, x, fidelities, levels, u_or_f = u_or_f)
...
if __name__ == "__main__":
    ...
    experiment_6_dropout_bias(u_or_f = 'u')
