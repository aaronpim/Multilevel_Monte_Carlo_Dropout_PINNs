...
from src.trainmodel import load_all, set_seed, CONFIG_to_folder_path

def get_model(seed = 0):
    set_seed(seed = seed)
    CONFIG = make_config(seed = 0)
    model, _, _, _, _, _, _, _ = load_all(CONFIG, device = 'cpu')
    model_path = os.path.join(CONFIG_to_folder_path(CONFIG), 'model.pt')
    model.load_state_dict(torch.load(model_path, map_location='cpu', weights_only = True))
    model.train()
    return model

def random_inputs(num_pts= 16):
    inputs = torch.randn([num_pts, 2])
    return inputs

def evaluate_model(model = None, num_evals = 16, num_pts = 17):
    if model is None:
        model = get_model()
    output = []
    for _ in range(num_evals):
        input_pairs = random_inputs(num_pts)
...
def main_experiment(fid_list = [8, 16, 32, 64, 128, 256, 512], num_reps = 100, model_seed = 0):
    ...
    with torch.no_grad():
        ...
        for fid in fid_list:
            ...
            results  = temp_vec.var(dim=[0,1]).mean()
            output_vec.append([fid, results])
        output_vec = torch.tensor(output_vec)
        torch.save(output_vec, f'plots/model_diff_evals.pt')

def plot_result(file_name = f'plots/model_diff_evals.pt'):
    output_vec = torch.load(file_name, weights_only = True)
