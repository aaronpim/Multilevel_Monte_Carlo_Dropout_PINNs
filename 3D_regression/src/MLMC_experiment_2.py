...
def reference_mean_and_variance(seed = 0, batch_size = 100, num_batches = 1000, filename = None):
    '''
    ...
    with torch.no_grad ():
        ...
        for i in range(num_batches):
            evals = torch.stack([ model(values).squeeze() for _ in range(batch_size)])
    ...
    running_mean = factor*(weights*(running_mean.sum(dim = [-1,-2,-3]))).sum()
    running_var  = factor*(weights*(running_var.sum( dim = [-1,-2,-3]))).sum()
    ...
    with torch.no_grad():
        ...
        for C in cost_multiplier:
            ...
            for seed in range(num_reps):
                set_seed(seed = seed)
                start_time = time.time()
                mean_estimator, _, sample_variance_estimator, _ = multi_level_multi_fidelity_eval(model, values, fidelity_ladder = fidelity_ladder, num_levels = num_levels, factor = factor, weights = weights)
