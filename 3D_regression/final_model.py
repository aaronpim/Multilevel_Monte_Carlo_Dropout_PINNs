...
def plot_exact(base_dir = 'model_plots'):
    os.makedirs(base_dir, exist_ok=True)
    ...
    CONFIG = make_config(seed = 0)
    _, _, data, _, _, _, _, _ = load_all(CONFIG, device = 'cpu')
    generic_plot(x,y,z, data.mean(dim = [0,1]), base_dir + '/exact_mean.png', alpha_power = 0.2)
